package com.ide.photoeditor.youshoot.cutomView.theme;
...
public class FinalMaskBitmap3D {
    ...
    public enum EFFECT {
        Roll2D_TB("Roll2D_TB") {
            public Bitmap getMask(Bitmap bottom, Bitmap top, int factor) {
                ...
                Bitmap mask = Bitmap.createBitmap(MyApp.VIDEO_WIDTH, MyApp.VIDEO_HEIGHT, Config.ARGB_8888);
    ...
    public static void setRotateDegree(int factor) {
        ...
        if (direction == 1) {
            ...
            axisY = (f1237f / ((float) i)) * ((float) MyApp.VIDEO_HEIGHT);
            return;
        }
        f1237f = rotateDegree;
        if (rollMode == EFFECT.Jalousie_BT || rollMode == EFFECT.Jalousie_LR) {
        ...
        axisX = (f1237f / ((float) i)) * ((float) MyApp.VIDEO_WIDTH);
    }

    public static void initBitmaps(Bitmap bottom, Bitmap top, EFFECT effect) {
        rollMode = effect;
        if (MyApp.VIDEO_HEIGHT > 0 || MyApp.VIDEO_WIDTH > 0) {
            bitmaps = (Bitmap[][]) Array.newInstance(Bitmap.class, new int[]{2, partNumber});
            averageWidth = MyApp.VIDEO_WIDTH / partNumber;
            averageHeight = MyApp.VIDEO_HEIGHT / partNumber;
            int i = 0;
            while (i < 2) {
                for (int j = 0; j < partNumber; j++) {
                    ...
                    if (rollMode == EFFECT.Jalousie_BT || rollMode == EFFECT.Jalousie_LR) {
                        if (direction == 1) {
                            rect = new Rect(0, averageHeight * j, MyApp.VIDEO_WIDTH, (j + 1) * averageHeight);
                            if (i == 0) {
                                bitmap = bottom;
                            } else {
                                bitmap = top;
                            ...
                            rect = new Rect(averageWidth * j, 0, (j + 1) * averageWidth, MyApp.VIDEO_HEIGHT);
                            if (i == 0) {
                                bitmap = bottom;
                            } else {
                                bitmap = top;
                        ...
                        partBitmap = getPartBitmap(i == 0 ? bottom : top, 0, averageHeight * j, new Rect(0, averageHeight * j, MyApp.VIDEO_WIDTH, (j + 1) * averageHeight));
                    }
    ...
    private static void drawRollWhole3D(Bitmap bottom, Bitmap top, Canvas canvas, boolean draw2D) {
        ...
        if (direction == 1) {
            ...
            camera.restore();
            matrix.preTranslate((float) ((-MyApp.VIDEO_WIDTH) / 2), 0.0f);
            matrix.postTranslate((float) (MyApp.VIDEO_WIDTH / 2), axisY);
            ...
            camera.getMatrix(matrix);
            camera.restore();
            matrix.preTranslate((float) ((-MyApp.VIDEO_WIDTH) / 2), (float) (-MyApp.VIDEO_HEIGHT));
            matrix.postTranslate((float) (MyApp.VIDEO_WIDTH / 2), axisY);
            ...
            camera.getMatrix(matrix);
            camera.restore();
            matrix.preTranslate(0.0f, (float) ((-MyApp.VIDEO_HEIGHT) / 2));
            matrix.postTranslate(axisX, (float) (MyApp.VIDEO_HEIGHT / 2));
            canvas.drawBitmap(currWholeBitmap, matrix, paint);
            camera.save();
            if (draw2D) {
                camera.rotateY(0.0f);
            ...
            camera.getMatrix(matrix);
            camera.restore();
            matrix.preTranslate((float) (-MyApp.VIDEO_WIDTH), (float) ((-MyApp.VIDEO_HEIGHT) / 2));
            matrix.postTranslate(axisX, (float) (MyApp.VIDEO_HEIGHT / 2));
            canvas.drawBitmap(nextWholeBitmap, matrix, paint);
        }
        canvas.restore();
    ...
    private static void drawRollInTurn(Canvas canvas) {
        for (int i = 0; i < partNumber; i++) {
            ...
            canvas.save();
            if (direction == 1) {
                float tAxisY = (tDegree / 90.0f) * ((float) MyApp.VIDEO_HEIGHT);
                if (tAxisY > ((float) MyApp.VIDEO_HEIGHT)) {
                    tAxisY = (float) MyApp.VIDEO_HEIGHT;
                }
                if (tAxisY < 0.0f) {
                    tAxisY = 0.0f;
                }
                ...
                float tAxisX = (tDegree / 90.0f) * ((float) MyApp.VIDEO_WIDTH);
                if (tAxisX > ((float) MyApp.VIDEO_WIDTH)) {
                    tAxisX = (float) MyApp.VIDEO_WIDTH;
                }
                if (tAxisX < 0.0f) {
                    tAxisX = 0.0f;
                }
